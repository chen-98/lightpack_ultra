...
| --- | --- | --- | --- | --- |
| 2026-05-16 | 核心服务可靠性: forgot-password 崩溃修复 | 在 `server/endpoints.js` 的 `POST /forgotPassword` 和 `POST /forgotUsername` 两个 handler 中新增 `!mailgun` 前置检查：当 Mailgun 未配置时立即返回 HTTP 503 + `{ errors: [{ message: "...请联系 920158928@qq.com..." }] }`，避免 `mailgun.messages()` 抛出 `TypeError: Cannot read properties of undefined` 导致 Node.js 进程崩溃并循环重启；前端 `.catch` 已能读取 `errors` 数组并展示具体提示 | `curl -sL -d '{"username":"cheny62"}' http://lighterpackultra.online/forgotPassword` 返回 503 + 可读错误消息，不再崩溃；`docker compose logs app` 无 TypeError | `fix: guard forgotPassword and forgotUsername against unconfigured mailgun` |
